...
const SETDAAP = 8
const SETART = 9
const PAIR_VERIFY_1 = 10
const PAIR_VERIFY_2 = 11
const OPTIONS2 = 12
const AUTH_SETUP = 13
const PAIR_PIN_START = 14
const PAIR_PIN_SETUP_1 = 15
const PAIR_PIN_SETUP_2 = 16
const PAIR_PIN_SETUP_3 = 17
const PAIR_SETUP_1 = 18
const PAIR_SETUP_2 = 19
const PAIR_SETUP_3 = 20
const PAIR_VERIFY_HAP_1 = 21
const PAIR_VERIFY_HAP_2 = 22
const SETUP_AP2_1 = 23
const SETUP_AP2_2 = 24
const SETPEERS = 25
const FLUSH = 26
const GETVOLUME = 27
const SETPROGRESS = 28
const OPTIONS3 = 29
...
Client.prototype.makeHead = function (method, uri, di, clear = false, dimode = null) {
  ...
  if (di) {
    let diResponse
    if (dimode == 'airplay2') {
      const ha1 = md5norm(di.username + ':' + di.realm + ':' + di.password)
      const ha2 = md5norm(method + ':' + uri)
      diResponse = md5(ha1 + ':' + di.nonce + ':' + ha2)
    }
    else {
...
Client.prototype.sendNextRequest = async function (di) {
  ...
  switch (this.status) {
    ...
    case SETPROGRESS:
      ...
      if (this.debug) {
        console.log('start', this.starttime, 'position', position, 'duration', duration, 'position1', hms(this.progress), 'duration1', hms(this.duration))
      }
    ...
    case SETART:
      ...
      else { this.socket.write(request) }
      request = ''
      break
...
Client.prototype.processData = function (blob, rawData) {
  ...
  switch (this.status) {
    ...
    case PAIR_VERIFY_HAP_2:
      ...
      if (this.debug) {
        console.log('write', this.credentials.writeKey)
    ...
    case OPTIONS:
      ...
      if (headers['WWW-Authenticate'] != null & response.code === 401) {
        ...
        const ha1 = md5norm(`${user}:${realm}:${pwd}`)
        const ha2 = md5norm(`${methodx}:${uri}`)
        const di_response = md5(`${ha1}:${nonce}:${ha2}`)
    ...
    case OPTIONS2:
      ...
      if (headers['WWW-Authenticate'] != null & response.code === 401) {
        ...
        let methodx = 'OPTIONS'
        let pwd = this.password
        const ha1 = md5(`${user}:${realm}:${pwd}`)
        const ha2 = md5(`${methodx}:${uri}`)
        const di_response = md5(`${ha1}:${nonce}:${ha2}`)
        this.code_digest = `Authorization: Digest username="${user}", realm="${realm}", nonce="${nonce}", uri="${uri}", response="${di_response}" \r\n\r\n`
